    ...
    <div className="flex-1 p-4 md:p-6 lg:p-10 overflow-y-auto h-screen relative z-10">
      <header className="mb-12 flex justify-between items-end backdrop-blur-sm">
        ...
        <div className="flex flex-col md:flex-row gap-2 md:gap-4">
          <button onClick={fetchEquipments} className="p-3 rounded-2xl bg-white/5 border border-white/10 text-white/50 hover:text-white transition-all">
      ...
      {/* Barra de búsqueda */}
      <div className="bg-white/5 border border-white/10 rounded-3xl p-4 md:p-5 lg:p-6 backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.3)] mb-8 flex flex-col md:flex-row gap-2 md:gap-4">
      ...
      {/* Tabla Principal */}
      <div className="bg-white/5 border border-white/10 rounded-3xl p-1 backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.3)] overflow-hidden overflow-x-auto">
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-white/5">
                  ...
                  {filteredData.length === 0 && !loading && (
                    <tr>
                      <td colSpan={5} className="p-16 text-center text-white/30 font-light tracking-wide italic">
          ...
          <div className="relative bg-[#0a0f1a] w-full max-w-6xl h-[85vh] rounded-[2rem] border border-white/10 shadow-[0_0_80px_rgba(0,0,0,0.6)] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-300">
            <div className="relative z-10 p-6 px-10 border-b border-white/5 flex justify-between items-center bg-white/5 backdrop-blur-md">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-3xl font-bold tracking-tight text-white/90">{selectedEquipment.equipo}</h3>
                  <p className="text-white/40 text-sm mt-1 font-light tracking-wide flex items-center gap-2">
              ...
              <div className="flex gap-4">
                <button onClick={() => { setEditEqData(selectedEquipment); setIsEditModalOpen(true); }} className="px-6 py-2.5 bg-gradient-to-r from-indigo-500 to-indigo-600 text-white rounded-xl text-sm font-semibold hover:scale-105 transition-all shadow-[0_0_20px_rgba(99,102,241,0.4)] border border-indigo-400/40 flex items-center gap-2">
                  <Edit size={16} /> Editar Ficha
                </button>
                <button onClick={handleDownloadPDF} className="px-6 py-2.5 bg-gradient-to-r from-orange-500 to-amber-500 text-white rounded-xl text-sm font-semibold hover:scale-105 transition-all shadow-[0_0_20px_rgba(245,158,11,0.4)] border border-orange-300/40 flex items-center gap-2">
            ...
            <div className="relative z-10 flex-1 overflow-y-auto p-4 md:p-6 lg:p-10 custom-scrollbar">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 lg:gap-8">
                ...
                <div className="space-y-4 md:space-y-6">
                   <div className="bg-white/5 border border-white/5 rounded-3xl p-4 md:p-5 lg:p-6">
                ...
                <div className="space-y-4 md:space-y-6">
                   <div className="bg-indigo-500/5 border border-indigo-500/20 rounded-3xl p-4 md:p-5 lg:p-6">
                      <h4 className="text-indigo-400 text-xs tracking-widest uppercase mb-5 pb-3 border-b border-white/5 font-bold">Localización y Clasificación</h4>
                      <div className="space-y-4">
                        ...
                        <div className="flex justify-between">
                          <span className="text-white/40 text-sm">Ubicación Especifica</span>
                          <span className="text-white text-sm font-medium">{selectedEquipment.ubicacion || 'N/A'}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-white/40 text-sm">Clase de Riesgo</span>
                ...
                <div className="space-y-4 md:space-y-6">
                   <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-3xl p-4 md:p-5 lg:p-6">
                      <h4 className="text-emerald-400 text-xs tracking-widest uppercase mb-5 pb-3 border-b border-white/5 font-bold">Calendario y Calidad</h4>
                      <div className="space-y-4">
                        <div>
                          <p className="text-white/30 text-[10px] uppercase font-light">Frecuencia Mtto.</p>
                          <p className="text-white text-sm">{selectedEquipment.frecuencia_mantenimiento || 'N/A'}</p>
                        </div>
                        <div className="bg-black/20 p-3 rounded-xl border border-white/5">
                          <p className="text-white/30 text-[10px] uppercase font-light mb-1">Vcto. Calibración</p>
                          <p className="text-rose-400 font-mono text-base font-bold">{selectedEquipment.fecha_vencimiento_calibracion || 'N/A'}</p>
                        </div>
                        <div>
                          <p className="text-white/30 text-[10px] uppercase font-light">Garantía</p>
                          <p className="text-white text-sm font-light">{selectedEquipment.garantia || 'N/A'}</p>
                        </div>
                      </div>
                   </div>
                </div>
